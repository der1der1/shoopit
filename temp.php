...
<body>
    <header class="col-pc-12 col-mobile-12">
        <nav id="run">
            ...
            $result1 = $mysqli->query('SELECT words FROM test1.`word ads 2` ORDER BY RAND()'); //抓廣告文的table
            //to再開資料庫
            //把MySQL的跑馬燈AD叫出來
        ...
        <nav id="tool">
            ...
            <div id="cart">
                <a href="temp.php"><img src="icon/cart-shopping-solid.svg" alt="cart icon" width="20px" height="20px" title="go to cart"><span></span></a>
                <?PHP
                echo 'Hi~ &nbsp; &nbsp;' . $user; ?>
            </div>
        </nav>
            ...
            <form method="post" enctype="multipart/form-data">
                ...
                while ($row2 = mysqli_fetch_row($result2)) {
                    ...
                    if (isset($_POST['choosing_' . $id . ''])) {
                        $buy_ornot = $buy_or_not + 1;
                        if ($buy_ornot % 2 == 0) {
                            $buy_or_not = 0;    //做是否選取購買的暫存boolean
                        } else {
                            $buy_or_not = 1;
                        }
                        echo  $id, $buy_or_not;
                        $sql = "UPDATE `test1`.`cart` SET `buy_bool` = '" . $buy_or_not . "' WHERE (`id` = '" . $id . "');";
                        $mysqli->query($sql);
                        $_SESSION["jumpto"] = $id;
                        // echo '<script>location.href = "temp.php #' . $id . '";</script>';
                        echo '<script>location.href = "temp.php";</script>';
                    }
                }
                $result2->close();   
            ...
            <form method="post" enctype="multipart/form-data">
                <div id="trading_bar">
                    ...
                    <button id="check" name="check" title="pay for my favorits">
                        <!-- <div id="check" title="pay for my favorits"> -->
...
if (isset($_POST['check'])) {
    if (mysqli_num_rows($result3) === 0) {
        ...
        // echo '<script>location.href = "temp.php";</script>';  //重置
        $result3->close();   
        $mysqli ->close();   //關閉資料庫
    } else {
        $result3->close();   
